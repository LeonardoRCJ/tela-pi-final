...
export default function FrequenciaAluno({ route, navigation }) {
  ...
  const stats = useMemo(() => {
    ...
    const presencas = historico.filter(item => item.presente === true).length;
    const faltas = totalAulas - presencas;
    const porcentagem = totalAulas > 0 ? Math.round((presencas / totalAulas) * 100) : 0;
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.statusCard}>
        ...
        <View style={styles.progressBar}>
          <View style={[
            styles.progressFill, 
            { width: `${stats.porcentagem}%`, backgroundColor: getColor(stats.porcentagem) }
          ]} />
        </View>
...
const styles = StyleSheet.create({
  ...
  statusCard: { backgroundColor: '#1A1A1A', margin: 20, padding: 25, borderRadius: 20, alignItems: 'center' },
  studentName: { color: '#FFF', fontWeight: '800', marginBottom: 10 },
  statLabel: { color: '#888' },
  statValue: { fontWeight: 'bold', marginTop: 5 },
  progressBar: { width: '100%', height: 10, backgroundColor: '#333', borderRadius: 10, marginTop: 15, overflow: 'hidden' },
  progressFill: { height: '100%' },
  miniStatsRow: { flexDirection: 'row', gap: 20, marginTop: 15 },
  miniStatText: { color: '#AAA', fontWeight: 'bold' },
  sectionTitle: { color: '#D4AF37', fontWeight: 'bold', marginLeft: 25, marginBottom: 15 },
  list: { paddingHorizontal: 20 },
  historyRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#161616', padding: 18, borderRadius: 15, marginBottom: 10 },
